...
func StreamConstantTable(ctx *ApiContext, args *TableRequest) (interface{}, int) {
	params := ctx.Params
	// access table
		...
		case "address": // expr-hash (!)
			...
			case pack.FilterModeEqual, pack.FilterModeNotEqual:
				...
				if err != nil || !hash.IsValid() {
					panic(EBadRequest(EC_PARAM_INVALID, fmt.Sprintf("invalid exprhash '%s'", val[0]), err))
				}
				q.Conditions.AddAndCondition(&pack.Condition{
			...
			case pack.FilterModeIn, pack.FilterModeNotIn:
				...
				for _, v := range strings.Split(val[0], ",") {
					...
					if err != nil || !addr.IsValid() {
						panic(EBadRequest(EC_PARAM_INVALID, fmt.Sprintf("invalid exprhash '%s'", v), err))
					}
