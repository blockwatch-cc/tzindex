...
func init() {
	...
	bigmapValueSourceNames["hash"] = "-"
	bigmapValueSourceNames["time"] = "h"
	bigmapValueAllAliases = append(bigmapValueAllAliases, "hash")
		...
		case "time":
			...
			case pack.FilterModeRange:
				...
				if from.After(bestTime) {
					panic(server.EBadRequest(server.EC_PARAM_INVALID, fmt.Sprintf("invalid future time %s", from), nil))
				}
				if to.After(bestTime) {
					panic(server.EBadRequest(server.EC_PARAM_INVALID, fmt.Sprintf("invalid future time %s", to), nil))
				}
			...
			case pack.FilterModeIn, pack.FilterModeNotIn:
				...
				for _, v := range cond.Value.([]time.Time) {
					if v.After(bestTime) {
						panic(server.EBadRequest(server.EC_PARAM_INVALID, fmt.Sprintf("invalid future time %s", v), nil))
					}
